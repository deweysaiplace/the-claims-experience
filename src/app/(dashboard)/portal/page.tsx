...
export default function PortalPage() {
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'scope':
      case 'roof-scope': return 'Roofing Scope'
      case 'reconciliation': return 'Reconciliation'
      case 'tasks': return 'Action Tasks'
      default: return type
    }
  }
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'field-note': return 'bg-rose-500/20 text-rose-300 border-rose-500/30'
      case 'field-scope': return 'bg-amber-600/20 text-amber-300 border-amber-600/30'
      case 'scope':
      case 'roof-scope': return 'bg-red-500/20 text-red-300 border-red-500/30'
      case 'reconciliation': return 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30'
      case 'tasks': return 'bg-indigo-500/20 text-indigo-300 border-indigo-500/30'
    ...
    <div className="max-w-6xl mx-auto space-y-6">
        ...
        <CardContent className="p-5 space-y-4">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-zinc-800 border border-zinc-700 rounded-xl pl-9 pr-4 py-2.5 text-sm text-white placeholder-zinc-500 outline-none focus:border-amber-600 transition-colors"
            />
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredReports.map(report => (
                <div 
                  ...
                  className="bg-zinc-800 border border-zinc-700 rounded-xl p-4 hover:border-zinc-500 cursor-pointer transition-colors group"
                >
      ...
      {selectedReport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <Card className="w-full max-w-4xl max-h-[90dvh] bg-zinc-900 border-zinc-700 flex flex-col shadow-2xl">
            <CardHeader className="flex flex-row items-center justify-between p-4 border-b border-zinc-800 shrink-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setSelectedReport(null)} className="p-1.5 rounded-lg text-zinc-500 hover:text-white hover:bg-zinc-800 transition-colors ml-2">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </CardHeader>
            <CardContent className="p-6 overflow-y-auto">
              <div className="text-zinc-200 prose prose-invert prose-base max-w-none prose-table:text-sm prose-headings:text-emerald-400 prose-headings:mt-6 prose-headings:mb-3 prose-p:text-zinc-200 prose-li:text-zinc-200 prose-strong:text-white prose-td:border-zinc-700 prose-th:border-zinc-700">
